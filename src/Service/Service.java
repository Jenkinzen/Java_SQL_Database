...
public class Service {
    ...
    public static ArrayList<Customers> searchInColumnDynamical(ArrayList<Customers> customerlist,int soughtAttribute,String input){
        ...
        for(int i = 0; i < customerlist.size();i++){
            for (int i2 = 0; i2 < customerlist.get(i).getInfoDynamical(soughtAttribute).size(); i2++) {
                if(customerlist.get(i).getInfoDynamical(soughtAttribute).get(i2) != null)
                {
                    if (customerlist.get(i).getInfoDynamical(soughtAttribute).get(i2).toLowerCase().contains(input.toLowerCase())) { // if ID of customer contains input
                        outputlist.add(customerlist.get(i));                           // put it in outputlist
                    }
                }
            }
        }
   ...
   //all_customers.get(0) [Customers 0] .getInfoDynamical(2 = first name) ->ArrayList<String> mit treffern
    ...
    public static HashMap<String, ArrayList<Double>> getBills(HashMap<String, ArrayList<Double>> customerlist, String idofcustomer, Double billofcustomer) {
        if (!customerlist.containsKey(idofcustomer)) {
            customerlist.put(idofcustomer, new ArrayList<>());
        } else {
            customerlist.get(idofcustomer).add(billofcustomer); // pack rinn.
        }
